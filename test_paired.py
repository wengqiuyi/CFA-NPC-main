...
# Optional TTA
# --------------------------------------------------------------------------- #
def tta_forward(model, x1: torch.Tensor, x2: torch.Tensor, threshold: float) -> np.ndarray:
...
def main():
    ...
    with torch.no_grad():
        for step, batch in enumerate(test_loader):
            ...
            if opt.tta:
                probs = tta_forward(model, x1, x2, opt.threshold)
            else:
            ...
            for b in range(pred_bin.shape[0]):
                m = compute_metrics(pred_bin[b, 0], gt_bin[b, 0])
                for k in metric_keys:
                    accum[k].append(m[k])
                if gt_bin[b, 0].sum() == 0: n_neg_only += 1
                else: n_pos_only += 1
                ...
                if opt.save_dir:
                    ...
                    import cv2
                    cv2.imwrite(os.path.join(opt.save_dir, f'sample_{step:04d}_{b:02d}.png'), p8)
